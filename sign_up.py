...
class Sign_Up(object):
            ...
            while (sign_up_state != True):
                ...
                try:
                    ...
                    if compromise[0]:
                        ...
                        from interface.ask_question import Ui_question
    ...
    def open_login_page(self):
        try:
            self.logger.info("Affichage de la page de connexion (login.py) depuis la page de création de compte (sign_up.py)")
            # Créer et afficher la fenêtre de login
            from login import Ui_Login
            self.window = QtWidgets.QDialog()
            login_ui = Ui_Login()  # Création de l'instance de la classe Login_Graph
            login_ui.setupUi(self.window)  # Configuration de la fenêtre de login
            self.dialog.close()  # Fermer la fenêtre actuelle (inscription)
            self.window.show()  # Afficher la fenêtre de login
